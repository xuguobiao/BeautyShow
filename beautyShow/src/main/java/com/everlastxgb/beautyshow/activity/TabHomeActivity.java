...
public class TabHomeActivity extends BaseTabActivity {
    ...
    private GirlListModel girlListModel = new GirlListModel(GirlModel.TYPE_GIRL);

    private AutoGridAdapter picTitleAdapter;

    private String defaultUrl = URLs.GIRLLIST_NEW;
    ...
    public static final int REQUEST_CODE_FIND = 10021;

    public static TabHomeActivity Self;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        titlebarBackView.setImageResource(R.drawable.selector_search);
        titlebarBackView.setOnClickListener(new View.OnClickListener() {

            @Override
            public void onClick(View v) {
                Intent intent = new Intent(self(), SearchInputActivity.class);

                intent.putExtra(KEY_MODEL, conditionModel);
        ...
        postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                manuPullDown(pullToRefreshView); // 第一次启动有时候触发不了下拉
            }
        }, MS_DELAY_REFRESH);
    ...
    private void initGridView() {
        ...
        picTitleAdapter = new AutoGridAdapter(this, girlListModel, pullToRefreshView.getRefreshableView());
        ...
        pullToRefreshView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
                GirlModel girlModel = girlListModel.getList().get(arg2);
                Intent intent = new Intent(TabHomeActivity.this, GirlDetailActivity.class);
                intent.putExtra(GirlDetailActivity.KEY_MODEL, girlModel);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_CODE_FIND && resultCode == RESULT_OK) {
            defaultUrl = data.getStringExtra(KEY_URL);
            manuPullDown(pullToRefreshView);
        }
    }
    ...
    private void initActionButton(ItemListModel itemListModel) {
        floatingActionMenu.removeAllMenuButtons();
        for (final ItemModel itemModel : itemListModel.getList()) {
    ...
    // for (int i = 0; i < groupLayout.getChildCount(); i++) {
    // MultiLineRadioGroup radioGroup = (MultiLineRadioGroup)
    // groupLayout.getChildAt(i).findViewById(R.id.raidoGroup);
    // if (radioGroup.getCheckedItems() != null &&
    // radioGroup.getCheckedItems().length > 0) {
    // return true;
    // }
    // }
    // return false;
    ...
    private void startListThread(final boolean isLoadingNew) {
        ...
        ApiController.getGirlListModel(URLs.getValidUrl(url), sort, new MyResultCallback(self(), isLoadingNew));
    }
    ...
    public class MyResultCallback extends ResultCallback<GirlListModel> {
        ...
        public void onResult(GirlListModel listModel, boolean success, String errMsg) {
            ...
            if (success) {
                if (isLoadingNew) {
                    girlListModel.getList().clear();
                }
                girlListModel.getList().addAll(listModel.getList());
                ...
                conditionModel = listModel.getConditionModel();
                // int rightCatVisi = filterList.size() == 0 ? View.GONE :
                // View.VISIBLE;
                // titlebarRightView.setVisibility(rightCatVisi);
            } else {
                UIHelper.showToastShort(self(), errMsg + "");
            }
            int tipVisibility = girlListModel.getList().size() == 0 ? View.VISIBLE : View.GONE;
